...
/** 내보내기 형식은 한 번 배포되면 사용자 파일에 남는 계약이라(formatManuscriptMarkdown KDoc),
 * 전체 출력을 문자열 그대로 못 박는다 — 어떤 변경이든 이 테스트가 의도적 결정을 강제한다. */
class ManuscriptMarkdownTest {
    ...
    @Test
    fun formatsAtxHeadingsFromChapterIndexWithSingleBlankLineSeparation() {
        val text = formatManuscriptMarkdown(
            listOf(
    ...
    fun appendsTitleToHeadingWhenPresent() {
        val text = formatManuscriptMarkdown(listOf(chapter(0, "본문.", title = "과거의 흔적")))
