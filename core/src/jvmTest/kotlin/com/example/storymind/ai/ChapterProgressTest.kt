...
class ChapterProgressTest {
    ...
    @Test
    fun `re-ingesting a chapter drops a stale edge even when both endpoints survive`() {
        // The case a bare (from,to) edge could never retract without a full rebuild: 율 and 진성
        // both appear in 1장 AND 2장, so both nodes stay alive. But the 율-진성 relation was only
        // ever asserted by 2장. Re-ingesting 2장 without that relation must drop the edge — its
        // chapter provenance set empties out — while keeping both nodes.
        val progress = ChapterProgress(
            wikiEntries = listOf(
                WikiEntry("yul", SmBadgeType.Character, "율", "1장: 주인공\n2장: 재등장", "2장"),
                WikiEntry("jinseong", SmBadgeType.Character, "진성", "1장: 동료\n2장: 재등장", "2장"),
            ),
            nodes = listOf(
                GraphNode("yul", SmBadgeType.Character, "율", 30f, 30f),
                GraphNode("jinseong", SmBadgeType.Character, "진성", 70f, 70f),
            ),
            edges = listOf(GraphEdge("yul", "jinseong", chapters = setOf("2장"))),
            orphanIds = emptySet(),
        )
        val reIngestChapter2 = IngestResult(
            chapterSummary = "2장 요약 (관계 삭제)",
            wikiEntries = listOf(
                WikiEntry("yul", SmBadgeType.Character, "율", "관계 없는 재등장", "2장"),
                WikiEntry("jinseong", SmBadgeType.Character, "진성", "관계 없는 재등장", "2장"),
            ),
            nodes = listOf(
                GraphNode("yul", SmBadgeType.Character, "율", 0f, 0f),
                GraphNode("jinseong", SmBadgeType.Character, "진성", 0f, 0f),
            ),
            edges = emptyList(),
            orphanIds = setOf("yul", "jinseong"),
        )

        val merged = progress.merge(reIngestChapter2, "2장")

        assertEquals(setOf("yul", "jinseong"), merged.nodes.mapTo(mutableSetOf()) { it.id })
        assertTrue("the stale edge must be pruned once its sole chapter drops it", merged.edges.isEmpty())
        assertEquals(setOf("yul", "jinseong"), merged.orphanIds)
        // 위치 보존: 살아남은 노드 좌표는 유지된다.
        assertEquals(30f, merged.nodes.single { it.id == "yul" }.x)
    }

    @Test
    fun `an edge two chapters both assert survives re-ingesting only one of them`() {
        // 율-진성 was asserted by both 1장 and 2장. Re-ingesting 2장 without it must keep the edge
        // (1장 still asserts it), dropping only 2장 from its provenance — the edge analog of an
        // entity kept because another chapter still mentions it.
        val progress = ChapterProgress(
            wikiEntries = listOf(
                WikiEntry("yul", SmBadgeType.Character, "율", "1장: 주인공\n2장: 재등장", "2장"),
                WikiEntry("jinseong", SmBadgeType.Character, "진성", "1장: 동료\n2장: 재등장", "2장"),
            ...
            edges = listOf(GraphEdge("yul", "jinseong", chapters = setOf("1장", "2장"))),
            orphanIds = emptySet(),
        )
        val reIngestChapter2 = IngestResult(
            chapterSummary = "2장 요약 (관계 삭제)",
        ...
        assertEquals(1, merged.edges.size)
        assertEquals(setOf("1장"), merged.edges.single().chapters)
    }

    @Test
    fun `a legacy sentinel edge is never stripped by a re-ingest`() {
        // A v2-migrated edge carries the "?" sentinel instead of a real chapter label. No chapter
        // label equals "?", so re-ingesting any chapter must leave it intact — legacy edges behave
        // exactly as they did before v3, until a full rebuild replaces them with real provenance.
        val progress = ChapterProgress(
            wikiEntries = listOf(
                WikiEntry("yul", SmBadgeType.Character, "율", "1장: 주인공", "1장"),
                WikiEntry("jinseong", SmBadgeType.Character, "진성", "1장: 동료", "1장"),
            ),
            nodes = listOf(
                GraphNode("yul", SmBadgeType.Character, "율", 30f, 30f),
                GraphNode("jinseong", SmBadgeType.Character, "진성", 70f, 70f),
            ),
            edges = listOf(GraphEdge("yul", "jinseong", chapters = setOf("?"))),
            orphanIds = emptySet(),
        )
        val reIngestChapter1 = IngestResult(
            chapterSummary = "1장 요약 (수정판)",
            wikiEntries = listOf(
                WikiEntry("yul", SmBadgeType.Character, "율", "수정된 주인공", "1장"),
                WikiEntry("jinseong", SmBadgeType.Character, "진성", "수정된 동료", "1장"),
            ),
            nodes = listOf(
                GraphNode("yul", SmBadgeType.Character, "율", 0f, 0f),
                GraphNode("jinseong", SmBadgeType.Character, "진성", 0f, 0f),
        ...
        val merged = progress.merge(reIngestChapter1, "1장")

        assertEquals(1, merged.edges.size)
        assertEquals(setOf("?"), merged.edges.single().chapters)
    }

    @Test
    fun `merging stamps this chapter onto a fresh edge's provenance`() {
        // Incoming IngestResult edges are "raw" (empty chapters); merge stamps the chapter label,
        // mirroring how an incoming WikiEntry.desc arrives without its "N장: " prefix.
        val progress = ChapterProgress(
            wikiEntries = listOf(WikiEntry("yul", SmBadgeType.Character, "율", "설명", "1장")),
            nodes = listOf(GraphNode("yul", SmBadgeType.Character, "율", 50f, 50f)),
            edges = emptyList(),
        ...
        assertEquals(setOf("2장"), merged.edges.single().chapters)
    }
